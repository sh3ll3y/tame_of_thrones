...
class TestInputReader(unittest.TestCase):
    ...
    def test_if_correct_number_of_rows_are_returned_from_input_file(self):
        input_file = os.path.join(CURRENT_DIR, "test_input_files/test_input.txt")
        expected = 3
        actual = len(ir.read_input(input_file))
        self.assertEqual(expected, actual)

    def test_if_correct_strings_are_returned_with_secret_message_striped_for_white_spaces_at_ends(self):
        input_file = os.path.join(CURRENT_DIR, "test_input_files/test_input.txt")
    ...
    def test_if_False_is_returned_if_input_format_is_not_correct(self):
        input_file = os.path.join(CURRENT_DIR, "test_input_files/bad_test_input.txt")
        lines = ir.read_input(input_file)
        regex = test_config_data.get("std_input_format")
