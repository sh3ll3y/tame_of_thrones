...
class TestStringHandler(unittest.TestCase):
    ...
    def test_is_string_method_returns_False_when_input_is_not_of_type_string(
            self):
        input_string = 123
        result = sh.is_string(input_string)
        self.assertFalse(result)

    def test_if_is_string_method_returns_True_when_input_is_list_of_strings(
            self):
        input_string = ["hello", "hi"]
        result = sh.is_string(input_string)
        self.assertTrue(result)

    def test_if_is_string_method_returns_False_when_input_list_has_a_non_string_value(
            self):
    ...
    def test_if_True_is_returned_if_a_main_str_contains_all_the_letters_of_the_sub_string_in_any_order(
            self):
        main_str = "geektrust"
        sub_str = "trust"
    ...
    def test_if_False_is_returned_if_a_main_str_does_not_contain_all_the_letters_of_the_sub_string_in_any_order(
            self):
        main_str = "geektrust"
        sub_str = "trustx"
        result = sh.contains_sub_str_letters(main_str, sub_str)
